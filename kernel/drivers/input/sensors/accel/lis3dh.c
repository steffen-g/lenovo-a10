...
{	
	...
	{			
		...
		{LIS3DH_CTRL_REG6,0x02},	//HL_ACTIVE: 0: interrupt active high; 1: interrupt active low
	...
	result = sensor_write_reg(client, LIS3DH_CTRL_REG1,0x37);       
                if(result)
                {
                        printk("%s:line=%d,error\n",__func__,__LINE__);
                        return result;
	...
	if(sensor->pdata->irq_enable)
	{
		...
		result = sensor_write_reg(client, LIS3DH_CTRL_REG3, 0x10);//I1_AOI1 =1  if motion	
		if(result)
		{
			printk("%s:line=%d,error\n",__func__,__LINE__);
			return result;
