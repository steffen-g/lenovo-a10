/* drivers/misc/ec/modules/ec-led.c
 *
...
{	
	...
	DBG_EC("%s,enable=%d, name=%s\n", __func__,enable, ec->ops[EC_ID_LED]->name);
	return result;
}
...
{	
	int result = 0;
	struct input_dev *input_dev = ec->led.input_dev;
	...
	input_dev->name = ec->ops[EC_ID_LED]->name;
...
{	
	...
	DBG_EC("%s,name=%s\n", __func__,ec->ops[EC_ID_LED]->name);
	return result;
}
...
{
	if(ec->ops[EC_ID_LED]->active)
		ec->ops[EC_ID_LED]->active(ec, 0);
...
struct ec_operate ec_led_ops = {
	.name				= "ec_led",
	.ec_id				= EC_ID_LED,			//i2c id number
	.slave_addr			= 0x61,
...
static int __init ec_led_init(void)
{
...
static void __exit ec_led_exit(void)
{
	struct ec_operate *ops = ec_get_ops();
...
subsys_initcall_sync(ec_led_init);
module_exit(ec_led_exit);
