...
static ssize_t sensor_show_query(struct device *dev,
...
static ssize_t sensor_set_query(struct device *dev,
...
*/
    ...
    printk("sensor-magf: user query: %d\n", value);
    data = sensor_read_reg(client, value);
    printk("sensor-magf: user query: reg0x%x = 0x%x\n", value, data);

    return len;
}

static ssize_t sensor_show_enable_interrupt(struct device *dev,
                struct device_attribute *attr, char *buf)
{
//    struct i2c_client *client = to_i2c_client(dev);
//    struct synaptics_data *priv = i2c_get_clientdata(client);
...
static ssize_t sensor_set_enable_interrupt(struct device *dev,
                         struct device_attribute *attr, const char *buf,
                         size_t len)
{
    struct i2c_client *client = to_i2c_client(dev);
    unsigned int value;
    unsigned char data, data_save;	
    int err;
    ...
    data = sensor_read_reg(client, 0x1b);
    data_save = data;

    printk("sensor-magf: user write %d, ori reg=0x%x\n", value, data);	
    if(value == 1){
	data &= ~(0x80);	
	data |= 0x20;
	err = sensor_write_reg(client, 0x1b, data);
	printk("sensor-magf: first stage write 0x%x, err=%d\n", data, err);
	data |= (data_save & 0x80) ? 0x80: 0x00;
	err = sensor_write_reg(client, 0x1b, data);
	printk("sensor-magf: second stage write 0x%x, err=%d\n", data, err);
    }else if(value == 0){
	data &= ~(0x80);
	data &= ~(0x20);
	err = sensor_write_reg(client, 0x1b, data);
	...
	data |= (data_save & 0x80) ? 0x80:0x00;
	err = sensor_write_reg(client, 0x1b, data);
	printk("sensor-magf: write new reg value: 0x%x, err=%d\n", data, err);
    }
...
static DEVICE_ATTR(query, 0660, sensor_show_query, sensor_set_query);
static DEVICE_ATTR(enable, 0660, sensor_show_enable_interrupt, sensor_set_enable_interrupt);
...
#endif
	my_result = sysfs_create_group(&client->dev.kobj, &sensor_attribute_group);
    	if (my_result) {
        	pr_err("sensor-magf: sysfs_create_group returned err = %d. Abort.\n", my_result);
    	}else{
		pr_err("sensor-magf: sysfs_create_group success.\n");
	}	
...
static int sensor_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&client->dev.kobj, &sensor_attribute_group);	
