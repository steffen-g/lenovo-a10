...
static inline void rk29_bl_min_brightness_check(struct rk29_bl_info *rk29_bl_info)
{
	...
	if (rk29_bl_info->min_brightness < 0 || rk29_bl_info->min_brightness > BL_STEP)
		rk29_bl_info->min_brightness = MAX_BRIGHTNESS_CORRECT;//50
...
static inline void rk29_bl_max_brightness_check(struct rk29_bl_info *rk29_bl_info)
{
	if (rk29_bl_info->max_brightness <= 0 || rk29_bl_info->max_brightness > BL_STEP)
		rk29_bl_info->max_brightness = BL_STEP;//255
}	
...
static int rk29_bl_update_status(struct backlight_device *bl)
{
	...
	//min:1637 1856 30  back:997 118
	//if (brightness !=0)
	//divh =998-998*(brightness - rk29_bl_info->min_brightness) /(BL_STEP - rk29_bl_info->min_brightness);
	DBG("bl:id = %d, divh = %d, div_total=%d, brightness = %d\n", id,divh, div_total, brightness);
...
static int rk29_bl_get_brightness(struct backlight_device *bl)
{
	...
	if (ref) {
		...
		printk("rk29_bl_get_brightness = %d\n",BL_STEP-(BL_STEP*divh/div_total));
...
{		
	...
	printk("pwm_clk_rate= %d, divh=%d,pre_div=%d\n", pwm_clk_rate,divh, pre_div);
	clk_enable(pwm_clk);
	rk_pwm_setup(id, PWM_DIV, divh, div_total);

	rk29_bl->props.power = FB_BLANK_UNBLANK;
	rk29_bl->props.fb_blank = FB_BLANK_UNBLANK;
	rk29_bl->props.brightness = 50;//BL_STEP;
	...
	register_early_suspend(&bl_early_suspend);
		...
		if( 1 ==  get_battery_status()){		
			...
			printk("lower power but not charging,so power off \n");
			msleep(10);
		    kernel_power_off(); 		
		}		
		else		
