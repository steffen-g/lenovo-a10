...
//Not use the timer
static void headset_check_type(void)
{
	int level = 0;
	...
	if(level < 0)
		goto out;
	printk("headset_check_type read_gpio level = %d\n",level);
	if((level > 0 && pdata->Hook_down_type == HOOK_DOWN_LOW)
		|| (level == 0 && pdata->Hook_down_type == HOOK_DOWN_HIGH))
	{
#if defined(CONFIG_SND_SOC_RT5640) && defined(CONFIG_SOUND_EX_FUN)
		if(codec_ex_fun.codec_mic_switch)
			codec_ex_fun.codec_mic_switch(true);
#endif
...
static void headsetobserve_work(struct work_struct *work)
{
	...
	if(headset_info->headset_status == HEADSET_IN)
	{
		...
		if(codec_ex_fun.codec_set_hp)
			codec_ex_fun.codec_set_hp(1);
#endif
		...
		if (pdata->Hook_gpio) {
#if defined(CONFIG_SND_SOC_RT5640) && defined(CONFIG_SOUND_EX_FUN)
			if(codec_ex_fun.codec_headset_detect){
				codec_ex_fun.codec_headset_detect(true);
				msleep(400);
				headset_check_type();
				goto out;
			}
			...
			printk("lllllllllllllllllllllllllll\n");
			goto out;
		}
	}
...
#ifdef CONFIG_SOUND_EX_FUN
		if(codec_ex_fun.codec_set_hp)
			codec_ex_fun.codec_set_hp(0);
		if(codec_ex_fun.codec_mic_switch)
			codec_ex_fun.codec_mic_switch(0);
#endif
...
#if defined(CONFIG_SND_SOC_RT5640) && defined(CONFIG_SOUND_EX_FUN)
		if(codec_ex_fun.codec_headset_detect)
			codec_ex_fun.codec_headset_detect(false);
		if(codec_ex_fun.codec_mic_switch)
			codec_ex_fun.codec_mic_switch(0);
#endif			
		if(pdata->headset_in_type == HEADSET_IN_HIGH)
	...
	//printk("kkkkkkkkkkkkkkkkkkk,cur_headset_status =%d\n", headset_info->cur_headset_status);
	//rk28_send_wakeup_key();
	switch_set_state(&headset_info->sdev, headset_info->cur_headset_status);	
	#if defined(CONFIG_SND_RK_SOC_RK2928) || defined(CONFIG_SOC_RK3028)
	if (headset_info->headset_status == HEADSET_OUT)
	{
		mdelay(200);
		rk2928_codec_set_spk(HEADSET_OUT);
		gpio_set_value(pdata->Sw_mic_gpio, GPIO_LOW);
	}
	#endif
	DBG("%s:headset_info->cur_headset_status = %d\n",__func__,headset_info->cur_headset_status);

out:
	
//	printk("nnnnnnnnnnnnnnnnnnnnnnnnnn\n");
...
//	printk("ooooooooooooooooooooooooooooo\n");
	...
	if(codec_ex_fun.codec_mic_switch){
		if(headset->isMic)
			codec_ex_fun.codec_mic_switch(1);
		else
			codec_ex_fun.codec_mic_switch(0);
	}
#endif
	...
	if(headset_info->cur_headset_status == 1)
		gpio_set_value(pdata->Sw_mic_gpio, GPIO_HIGH);
	#endif
       //	rk28_send_wakeup_key();
	
	DBG("%s:send_wakeup_key \n",__func__);	
	switch_set_state(&headset_info->sdev, headset_info->cur_headset_status);
