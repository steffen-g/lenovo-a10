/* drivers/misc/ec/modules/ec-ps2.c
 *
...
{	
	...
	DBG_EC("%s,enable=%d, name=%s\n", __func__,enable, ec->ops[EC_ID_PS2]->name);
	return result;
}
...
{	
	int result = 0;
	struct input_dev *input_dev = ec->ps2.input_dev;
	...
	input_dev->name = ec->ops[EC_ID_PS2]->name;
...
{	
	...
	DBG_EC("%s,name=%s\n", __func__,ec->ops[EC_ID_PS2]->name);
	return result;
}
...
{
	if(ec->ops[EC_ID_PS2]->active)
		ec->ops[EC_ID_PS2]->active(ec, 0);
...
struct ec_operate ec_ps2_ops = {
	.name				= "ec_ps2",
	.ec_id				= EC_ID_PS2,			//i2c id number
	.slave_addr			= 0x61,
...
static int __init ec_ps2_init(void)
{
...
static void __exit ec_ps2_exit(void)
{
	struct ec_operate *ops = ec_get_ops();
...
subsys_initcall_sync(ec_ps2_init);
module_exit(ec_ps2_exit);
